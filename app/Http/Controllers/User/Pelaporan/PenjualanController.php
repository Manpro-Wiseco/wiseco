...
class PenjualanController extends Controller
{
    ...
    public function retur(Request $request)
    {
        ...
        if ($request->tanggal === "year") {
            // get penjualan table from database this year
            $data = ReturPenjualan::with(['penjualan', 'pelanggan'])->currentCompany()->whereYear('created_at', date('Y'))->get();
        } elseif ($request->tanggal === "month") {
            // get penjualan table from database this month
            $data = ReturPenjualan::with(['penjualan', 'pelanggan'])->currentCompany()->whereMonth('created_at', date('m'))->get();
        } elseif ($request->tanggal === "custom") {
            // get penjualan table from database this range
            $data = ReturPenjualan::with(['penjualan', 'pelanggan'])->currentCompany()->whereBetween('created_at', [$request->dari_tanggal, $request->hingga_tanggal])->get();
        } elseif ($request->tanggal === "week") {
            // get penjualan table from database this week
            $data = ReturPenjualan::with(['penjualan', 'pelanggan'])->currentCompany()->whereBetween('created_at', [Carbon::now()->startOfWeek(), Carbon::now()->endOfWeek()])->get();
        } else {
            // get penjualan table from database this date
            $data = ReturPenjualan::with(['penjualan', 'pelanggan'])->currentCompany()->whereDate('created_at', date('Y-m-d'))->get();
        }
        ...
        $pdf = PDF::loadView('user.pelaporan.penjualan.retur-pdf', compact('data', 'data_request', 'tanggal'));
        return $pdf->stream(Carbon::now()->format('Y-m-d') . '-riwayat-retur-penjualan.pdf');
    }
}
